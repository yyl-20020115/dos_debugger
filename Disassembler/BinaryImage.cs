...
{
    readonly SegmentCollection segments = [];
    readonly ByteAttributeCollection attributes;
    readonly InstructionCollection instructions;
    readonly XRefCollection crossReferences = [];
    readonly BasicBlockCollection basicBlocks = [];
    readonly ProcedureCollection procedures = [];
    readonly ErrorCollection errors = [];

    protected BinaryImage()
    {
        this.instructions = new (this);
    ...
    public virtual string FormatAddress(Address address)
        => $"seg{address.Segment:000}:{address.Offset:X4}";
    ...
    public SegmentCollection Segments => segments;

    public InstructionCollection Instructions => instructions;

    public XRefCollection CrossReferences => crossReferences;

    public BasicBlockCollection BasicBlocks => basicBlocks;

    public ProcedureCollection Procedures => procedures;

    public ErrorCollection Errors => errors;
...
public class ByteAttributeCollection(BinaryImage image)
{
    readonly List<ByteAttribute[]> attrs = [];
    readonly BinaryImage image = image;
...
public struct ByteAttribute
{
    ...
    {
        readonly get => (ByteType)(attr & 0x3);
        set => attr = (byte)((attr & ~3) | ((int)value & 3));
    }

    public bool IsLeadByte
    {
        readonly get => (attr & 0x08) != 0;
    ...
    public static readonly ByteAttribute Empty = new();
}
